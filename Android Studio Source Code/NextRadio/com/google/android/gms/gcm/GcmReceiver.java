...
public class GcmReceiver extends WakefulBroadcastReceiver {
    ...
    private void zzj(Context context, Intent intent) {
        if (isOrderedBroadcast()) {
            setResultCode(ActiveMQPrefetchPolicy.DEFAULT_QUEUE_BROWSER_PREFETCH);
        }
        zzk(context, intent);
        try {
    ...
    private void zzk(Context context, Intent intent) {
        ...
        if (!context.getPackageName().equals(serviceInfo.packageName) || serviceInfo.name == null) {
            Log.e("GcmReceiver", "Error resolving target intent service, skipping classname enforcement. Resolved service was: " + serviceInfo.packageName + ReadOnlyContext.SEPARATOR + serviceInfo.name);
            return;
        }
        String str = serviceInfo.name;
        if (str.startsWith(ActiveMQDestination.PATH_SEPERATOR)) {
