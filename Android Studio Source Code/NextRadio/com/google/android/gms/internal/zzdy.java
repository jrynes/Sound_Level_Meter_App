...
public class zzdy {
    ...
    private static void zza(String str, zzdz com_google_android_gms_internal_zzdz) {
        if (zzb.zzQ(2)) {
            zzin.m1674v(String.format(str, new Object[]{com_google_android_gms_internal_zzdz}));
        }
    }
    ...
    private String zzef() {
        try {
            ...
            while (it.hasNext()) {
                stringBuilder.append(Base64.encodeToString(((zzdz) it.next()).toString().getBytes(HttpRequest.CHARSET_UTF8), 0));
                if (it.hasNext()) {
                    stringBuilder.append(Stomp.NULL);
                }
            }
    ...
    void restore() {
        ...
        if (this.zzAz != null) {
            ...
            for (Object obj : zzY(sharedPreferences.getString("PoolKeys", Stomp.EMPTY))) {
