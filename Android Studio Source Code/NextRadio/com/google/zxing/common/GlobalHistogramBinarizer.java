...
public class GlobalHistogramBinarizer extends Binarizer {
    ...
    public BitArray getBlackRow(int y, BitArray row) throws NotFoundException {
        ...
        for (x = 0; x < width; x++) {
            int i = (localLuminances[x] & Type.ANY) >> LUMINANCE_SHIFT;
            localBuckets[i] = localBuckets[i] + 1;
        }
        int blackPoint = estimateBlackPoint(localBuckets);
        int left = localLuminances[0] & Type.ANY;
        int center = localLuminances[1] & Type.ANY;
        for (x = 1; x < width - 1; x++) {
            int right = localLuminances[x + 1] & Type.ANY;
    ...
    public BitMatrix getBlackMatrix() throws NotFoundException {
        ...
        for (y = 0; y < height; y++) {
            int offset = y * width;
            for (x = 0; x < width; x++) {
                if ((localLuminances[offset + x] & Type.ANY) < blackPoint) {
                    matrix.set(x, y);
                }
            }
        }
