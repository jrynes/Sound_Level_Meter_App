...
public final class StringUtils {
    ...
    public static String guessEncoding(byte[] bytes, Map<DecodeHintType, ?> hints) {
        ...
        if (r0 > 3 && bytes[0] == -17 && bytes[1] == -69 && bytes[2] == -65) {
            return UTF8;
        }
        ...
        int i = 0;
        while (i < length2 && (canBeISO88591 || canBeShiftJIS || canBeUTF8)) {
            int nextValue;
            int value = bytes[i] & Type.ANY;
            ...
            if ((value == 194 || value == 195) && i < length2 - 1) {
                nextValue = bytes[i + 1] & Type.ANY;
        ...
        if (canBeShiftJIS && (maybeDoubleByteCount >= 3 || maybeSingleByteKatakanaCount * 20 > length2)) {
            return SHIFT_JIS;
        }
