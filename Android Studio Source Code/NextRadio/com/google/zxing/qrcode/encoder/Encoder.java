...
public final class Encoder {
    ...
    private static boolean isOnlyDoubleByteKanji(String content) {
        try {
            ...
            for (int i = 0; i < length; i += 2) {
                int byte1 = bytes[i] & Type.ANY;
                if ((byte1 < Service.PWDGEN || byte1 > 159) && (byte1 < 224 || byte1 > 235)) {
                    return false;
                }
            }
            return true;
    ...
    static byte[] generateECBytes(byte[] dataBytes, int numEcBytesInBlock) {
        ...
        for (i = 0; i < numDataBytes; i++) {
            toEncode[i] = dataBytes[i] & Type.ANY;
        }
    ...
    static void appendBytes(String content, Mode mode, BitArray bits, String encoding) throws WriterException {
        switch (C10741.$SwitchMap$com$google$zxing$qrcode$decoder$Mode[mode.ordinal()]) {
            case Zone.PRIMARY /*1*/:
    ...
    static void appendKanjiBytes(String content, BitArray bits) throws WriterException {
        try {
            ...
            int length = bytes.length;
            for (int i = 0; i < length; i += 2) {
                int byte2 = bytes[i + 1] & Type.ANY;
                int code = ((bytes[i] & Type.ANY) << 8) | byte2;
                int subtracted = -1;
                ...
                bits.appendBits(((subtracted >> 8) * 192) + (subtracted & Type.ANY), 13);
            }
        } catch (UnsupportedEncodingException uee) {
            throw new WriterException(uee.toString());
        }
