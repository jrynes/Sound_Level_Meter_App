...
final class DecodedBitStreamParser {
    ...
    private static void decodeHanziSegment(BitSource bits, StringBuilder result, int count) throws FormatException {
        ...
        while (count > 0) {
            ...
            buffer[offset] = (byte) ((assembledTwoBytes >> 8) & Type.ANY);
            buffer[offset + GB2312_SUBSET] = (byte) (assembledTwoBytes & Type.ANY);
            offset += 2;
            count--;
        }
        try {
    ...
    private static void decodeNumericSegment(BitSource bits, StringBuilder result, int count) throws FormatException {
        while (count >= 3) {
            ...
            if (threeDigitsBits >= ActiveMQPrefetchPolicy.DEFAULT_QUEUE_PREFETCH) {
                throw FormatException.getFormatInstance();
            }
    ...
    private static int parseECIValue(BitSource bits) {
        ...
        if ((firstByte & Flags.FLAG8) == 0) {
            return firstByte & Service.LOCUS_CON;
        }
        if ((firstByte & 192) == Flags.FLAG8) {
