...
final class DecodedBitStreamParser {
    ...
    static /* synthetic */ class C10671 {
        static final /* synthetic */ int[] f2076xb73eb560;

        static {
            f2076xb73eb560 = new int[Mode.values().length];
            try {
                f2076xb73eb560[Mode.C40_ENCODE.ordinal()] = 1;
            } catch (NoSuchFieldError e) {
            }
            try {
                f2076xb73eb560[Mode.TEXT_ENCODE.ordinal()] = 2;
            } catch (NoSuchFieldError e2) {
            }
            try {
                f2076xb73eb560[Mode.ANSIX12_ENCODE.ordinal()] = 3;
            } catch (NoSuchFieldError e3) {
            }
            try {
                f2076xb73eb560[Mode.EDIFACT_ENCODE.ordinal()] = 4;
            } catch (NoSuchFieldError e4) {
            }
            try {
                f2076xb73eb560[Mode.BASE256_ENCODE.ordinal()] = 5;
    ...
    static {
        ...
        C40_SHIFT2_SET_CHARS = new char[]{'!', '\"', '#', '$', '%', '&', '\'', '(', ')', '*', '+', ActiveMQDestination.COMPOSITE_SEPERATOR, '-', '.', '/', ':', ';', '<', '=', '>', '?', '@', '[', '\\', ']', '^', '_'};
    ...
    static DecoderResult decode(byte[] bytes) throws FormatException {
        ...
        do {
            if (mode == Mode.ASCII_ENCODE) {
                ...
                switch (C10671.f2076xb73eb560[mode.ordinal()]) {
                    case Zone.PRIMARY /*1*/:
    ...
    private static Mode decodeAsciiSegment(BitSource bits, StringBuilder result, StringBuilder resultTrailer) throws FormatException {
        ...
        do {
            ...
            if (oneByte == 0) {
                throw FormatException.getFormatInstance();
            } else if (oneByte <= Flags.FLAG8) {
                if (upperShift) {
                    oneByte += Flags.FLAG8;
                }
                result.append((char) (oneByte - 1));
                return Mode.ASCII_ENCODE;
            } else if (oneByte == Service.PWDGEN) {
                ...
                if (oneByte <= 229) {
                    ...
                    if (oneByte == 232) {
                        ...
                        if (oneByte == 235) {
                            ...
                            if (!(oneByte == 241 || oneByte < 242 || (oneByte == Type.MAILA && bits.available() == 0))) {
                                throw FormatException.getFormatInstance();
                            }
                        }
                    }
                    ...
                    switch (shift) {
                        case Tokenizer.EOF /*0*/:
                            if (cValue < 3) {
                                ...
                                result.append((char) (c40char + Flags.FLAG8));
                                upperShift = false;
                                break;
                            } else {
                                throw FormatException.getFormatInstance();
                        ...
                        case Zone.PRIMARY /*1*/:
                            if (upperShift) {
                                result.append((char) (cValue + Flags.FLAG8));
                                upperShift = false;
                            } else {
                    ...
                    switch (shift) {
                        case Tokenizer.EOF /*0*/:
                            if (cValue < 3) {
                                ...
                                result.append((char) (textChar + Flags.FLAG8));
                                upperShift = false;
                                break;
                            } else {
                                throw FormatException.getFormatInstance();
    ...
    private static void decodeBase256Segment(BitSource bits, StringBuilder result, Collection<byte[]> byteSegments) throws FormatException {
        ...
        } else if (d1 < Type.TSIG) {
            count = d1;
            codewordPosition = codewordPosition2;
        } else {
            codewordPosition = codewordPosition2 + 1;
            count = ((d1 - 249) * Type.TSIG) + unrandomize255State(bits.readBits(8), codewordPosition2);
        }
        if (count < 0) {
            throw FormatException.getFormatInstance();
        }
    ...
    private static int unrandomize255State(int randomizedBase256Codeword, int base256CodewordPosition) {
        int tempVariable = randomizedBase256Codeword - (((base256CodewordPosition * 149) % Type.ANY) + 1);
        return tempVariable >= 0 ? tempVariable : tempVariable + KEYRecord.OWNER_ZONE;
    }
}
