...
public final class Detector {
    ...
    private static void correctCodeWordVertices(ResultPoint[] vertices, boolean upsideDown) {
        ...
        if (skew > 2.0f) {
            ...
            deltax = vertices[SKEW_THRESHOLD].getX() - vertices[4].getX();
            vertices[6] = new ResultPoint(vertices[6].getX(), vertices[6].getY() - (((vertices[SKEW_THRESHOLD].getX() - vertices[6].getX()) * (vertices[SKEW_THRESHOLD].getY() - vertices[4].getY())) / deltax));
        }
    ...
    private static float computeModuleWidth(ResultPoint[] vertices) {
        return (((ResultPoint.distance(vertices[0], vertices[4]) + ResultPoint.distance(vertices[1], vertices[5])) / 34.0f) + ((ResultPoint.distance(vertices[6], vertices[SKEW_THRESHOLD]) + ResultPoint.distance(vertices[7], vertices[3])) / 36.0f)) / 2.0f;
    }
        ...
        for (int x = column; x < column + width; x++) {
            if ((matrix.get(x, row) ^ isWhite) != 0) {
                ...
                if (counterPosition != patternLength - 1) {
                    ...
                    int[] iArr = new int[SKEW_THRESHOLD];
                    iArr[0] = patternStart;
                    iArr[1] = x;
                    return iArr;
                } else {
                    patternStart += counters[0] + counters[1];
                    System.arraycopy(counters, SKEW_THRESHOLD, counters, 0, patternLength - 2);
