...
class Assets {
    ...
    L_0x0024:
        ...
        r6.m638d(r7, r8);	 Catch:{ all -> 0x0069 }
    ...
    private void writeAssetToDisk(CircuitBreaker breaker, byte[] bytes, String filename) {
        if (!breaker.isBroken()) {
            ...
            if (fileOutputHandler == null) {
                this.logger.m639e("Could not create output handler for asset file.");
                return;
            }
            if (fileOutputHandler.open(WriteMethod.OVERWRITE)) {
                try {
                    ...
                    this.logger.m646w("Caught IOException while writing asset to disk: %s", filename);
