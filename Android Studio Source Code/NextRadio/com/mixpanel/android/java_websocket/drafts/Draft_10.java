...
public class Draft_10 extends Draft {
    ...
    public ByteBuffer createBinaryFrame(Framedata framedata) {
        ...
        int sizebytes = mes.remaining() <= Service.LOCUS_MAP ? 1 : mes.remaining() <= Message.MAXLENGTH ? 2 : 8;
        if (sizebytes > 1) {
            i = sizebytes + 1;
        } else {
            i = sizebytes;
        ...
        if ($assertionsDisabled || payloadlengthbytes.length == sizebytes) {
            if (sizebytes == 1) {
                ...
                buf.put((byte) ((mask ? -128 : 0) | Service.LOCUS_CON));
                buf.put(payloadlengthbytes);
            } else {
