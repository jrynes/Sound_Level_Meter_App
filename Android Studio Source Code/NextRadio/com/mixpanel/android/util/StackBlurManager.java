...
public class StackBlurManager {
    public static void process(Bitmap source, int radius) {
        if (radius >= 1) {
            ...
            int divsum = (div + 1) >> 1;
            divsum *= divsum;
            int[] dv = new int[(divsum * KEYRecord.OWNER_ZONE)];
            for (i = 0; i < divsum * KEYRecord.OWNER_ZONE; i++) {
                dv[i] = i / divsum;
            }
            int yw = 0;
            int yi = 0;
            ...
            for (x = 0; x < width; x++) {
                ...
                stackpointer = radius;
                for (y = 0; y < height; y++) {
                    currentPixels[yi] = ((ViewCompat.MEASURED_STATE_MASK | (dv[rsum] << 16)) | (dv[gsum] << 8)) | dv[bsum];
                    rsum -= routsum;
                    gsum -= goutsum;
                    bsum -= boutsum;
                    sir = stack[((stackpointer - radius) + div) % div];
