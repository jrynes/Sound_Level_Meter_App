...
public class ImageLoader {
    ...
    public synchronized void init(ImageLoaderConfiguration configuration) {
        if (configuration == null) {
            ...
            if (configuration.writeLogs) {
                C1271L.m1945d(LOG_INIT_CONFIG, new Object[0]);
            }
            this.engine = new ImageLoaderEngine(configuration);
            this.configuration = configuration;
        } else {
            C1271L.m1950w(WARNING_RE_INIT_CONFIG, new Object[0]);
        }
    }
    ...
    public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options, ImageLoadingListener listener) {
        ...
        if (bmp == null || bmp.isRecycled()) {
            ...
            LoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(this.engine, new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey, options, listener, this.engine.getLockForUri(uri)), options.getHandler());
        ...
        if (this.configuration.writeLogs) {
            C1271L.m1945d(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE, memoryCacheKey);
        }
        if (options.shouldPostProcess()) {
            ProcessAndDisplayImageTask displayTask2 = new ProcessAndDisplayImageTask(this.engine, bmp, new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey, options, listener, this.engine.getLockForUri(uri)), options.getHandler());
            if (options.isSyncLoading()) {
                displayTask2.run();
                return;
            } else {
        ...
        listener.onLoadingComplete(uri, imageAware.getWrappedView(), options.getDisplayer().display(bmp, imageAware, LoadedFrom.MEMORY_CACHE));
    }
    ...
    public void loadImage(String uri, ImageSize targetImageSize, DisplayImageOptions options, ImageLoadingListener listener) {
        ...
        displayImage(uri, new ImageNonViewAware(targetImageSize, ViewScaleType.CROP), options, listener);
    }
    ...
    public void destroy() {
        if (this.configuration != null && this.configuration.writeLogs) {
            C1271L.m1945d(LOG_DESTROY, new Object[0]);
        }
        stop();
        this.engine = null;
