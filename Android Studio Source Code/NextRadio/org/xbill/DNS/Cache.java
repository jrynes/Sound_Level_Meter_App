...
public class Cache {
    ...
    public SetResponse addMessage(Message in) {
        ...
        while (true) {
            ...
            if (auth[i].getType() == 6) {
                if (curname.subdomain(auth[i].getName())) {
                    soa = auth[i];
                    i++;
                }
            }
        ...
        while (true) {
            ...
            if (type == 1 || type == 28 || type == 38) {
                if (additionalNames.contains(addl[i].getName())) {
                    cred = getCred(3, isAuth);
