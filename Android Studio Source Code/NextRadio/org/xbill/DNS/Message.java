...
public class Message implements Cloneable {
            ...
            try {
                ...
                for (int j = TSIG_UNSIGNED; j < count; j += TSIG_VERIFIED) {
                    ...
                    if (i == TSIG_SIGNED) {
                        ...
                        if (rec.getType() == 24) {
                            if (((SIGRecord) rec).getTypeCovered() == 0) {
                                this.sig0start = pos;
                            }
                        }
                    }
    ...
    public String toString() {
        StringBuffer sb = new StringBuffer();
        if (getOPT() != null) {
            sb.append(new StringBuffer().append(this.header.toStringWithRcode(getRcode())).append(Stomp.NEWLINE).toString());
        } else {
            sb.append(new StringBuffer().append(this.header).append(Stomp.NEWLINE).toString());
        }
        if (isSigned()) {
            sb.append(";; TSIG ");
            if (isVerified()) {
        ...
        for (int i = TSIG_UNSIGNED; i < TSIG_FAILED; i += TSIG_VERIFIED) {
            ...
            sb.append(new StringBuffer().append(sectionToString(i)).append(Stomp.NEWLINE).toString());
        }
