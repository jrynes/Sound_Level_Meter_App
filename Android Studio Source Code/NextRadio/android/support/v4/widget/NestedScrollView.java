...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild, ScrollingView {
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        if (canScroll()) {
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    ...
                    case Service.FTP_DATA /*20*/:
                        if (!event.isAltPressed()) {
                            handled = arrowScroll(Service.CISCO_FNA);
                            break;
                        }
                        handled = fullScroll(Service.CISCO_FNA);
                        break;
                    case Protocol.CFTP /*62*/:
                        pageScroll(event.isShiftPressed() ? 33 : Service.CISCO_FNA);
                        break;
                }
            }
            return handled;
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, Service.CISCO_FNA);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(Service.CISCO_FNA)) {
                return false;
            }
            return true;
        }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (nextFocused == null || !isWithinDeltaOfScreen(nextFocused, maxJump, getHeight())) {
            ...
            } else if (direction == Service.CISCO_FNA && getChildCount() > 0) {
