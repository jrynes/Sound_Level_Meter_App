...
public class FileProvider extends ContentProvider {
    ...
    static class SimplePathStrategy implements PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            try {
                ...
                if (rootPath.endsWith(ReadOnlyContext.SEPARATOR)) {
                    path = path.substring(rootPath.length());
                } else {
                    path = path.substring(rootPath.length() + 1);
                }
                return new Builder().scheme("content").authority(this.mAuthority).encodedPath(Uri.encode((String) mostSpecific.getKey()) + '/' + Uri.encode(path, ReadOnlyContext.SEPARATOR)).build();
            } catch (IOException e) {
    ...
    static {
        ...
        DEVICE_ROOT = new File(ReadOnlyContext.SEPARATOR);
        sCache = new HashMap();
    }
    ...
    public String getType(Uri uri) {
        ...
        return ActiveMQBlobMessage.BINARY_MIME_TYPE;
    }
    ...
    private static PathStrategy parsePathStrategy(Context context, String authority) throws IOException, XmlPullParserException {
        ...
        XmlResourceParser in = context.getPackageManager().resolveContentProvider(authority, Flags.FLAG8).loadXmlMetaData(context.getPackageManager(), META_DATA_FILE_PROVIDER_PATHS);
        if (in == null) {
