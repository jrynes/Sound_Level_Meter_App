...
public class QueueFile implements Closeable {
    ...
    class 1 implements ElementReader {
        boolean first;
    ...
    public QueueFile(File file) throws IOException {
        this.buffer = new byte[HEADER_LENGTH];
        if (!file.exists()) {
            initialize(file);
        }
        this.raf = open(file);
    ...
    private static void writeInt(byte[] buffer, int offset, int value) {
        ...
        buffer[offset + 1] = (byte) (value >> HEADER_LENGTH);
    ...
    private static int readInt(byte[] buffer, int offset) {
        return ((((buffer[offset] & Type.ANY) << 24) + ((buffer[offset + 1] & Type.ANY) << HEADER_LENGTH)) + ((buffer[offset + 2] & Type.ANY) << 8)) + (buffer[offset + 3] & Type.ANY);
    }
    ...
    private int wrapPosition(int position) {
        return position < this.fileLength ? position : (position + HEADER_LENGTH) - this.fileLength;
    }
    ...
    public int usedBytes() {
        ...
        if (this.last.position >= this.first.position) {
            return (((this.last.position - this.first.position) + 4) + this.last.length) + HEADER_LENGTH;
        }
    ...
    public synchronized void peek(ElementReader reader) throws IOException {
        if (this.elementCount > 0) {
            reader.read(new ElementInputStream(this.first, null), this.first.length);
        }
    }
