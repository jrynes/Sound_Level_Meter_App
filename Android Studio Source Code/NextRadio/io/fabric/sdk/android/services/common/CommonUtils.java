...
public class CommonUtils {
    ...
    enum Architecture {
        ...
        static {
            matcher = new HashMap(CommonUtils.DEVICE_STATE_DEBUGGERATTACHED);
...
*/
    ...
    static {
        ...
        totalRamInBytes = UNCALCULATED_TOTAL_RAM;
    ...
    public static String streamToString(InputStream is) throws IOException {
        ...
        return s.hasNext() ? s.next() : Stomp.EMPTY;
    }
    ...
    public static String createInstanceIdFrom(String... sliceIds) {
        ...
        String[] arr$ = sliceIds;
        int len$ = arr$.length;
        for (int i$ = 0; i$ < len$; i$ += DEVICE_STATE_ISSIMULATOR) {
            String id = arr$[i$];
            if (id != null) {
                sliceIdList.add(id.replace("-", Stomp.EMPTY).toLowerCase(Locale.US));
            }
        }
    ...
    public static boolean getProximitySensorEnabled(Context context) {
        if (isEmulator(context) || ((SensorManager) context.getSystemService("sensor")).getDefaultSensor(DEVICE_STATE_BETAOS) == null) {
            return CLS_TRACE_DEFAULT;
        }
        return true;
    }
    ...
    public static boolean isClsTrace(Context context) {
        if (clsTrace == null) {
            clsTrace = Boolean.valueOf(getBooleanResourceValue(context, CLS_TRACE_PREFERENCE_NAME, CLS_TRACE_DEFAULT));
        }
    ...
    public static boolean isEmulator(Context context) {
        return (SDK.equals(Build.PRODUCT) || GOOGLE_SDK.equals(Build.PRODUCT) || Secure.getString(context.getContentResolver(), "android_id") == null) ? true : CLS_TRACE_DEFAULT;
    }
    ...
    public static int getDeviceState(Context context) {
        ...
        if (isEmulator(context)) {
            deviceState = 0 | DEVICE_STATE_ISSIMULATOR;
        }
        if (isRooted(context)) {
            deviceState |= DEVICE_STATE_JAILBROKEN;
        }
        if (isDebuggerAttached()) {
            return deviceState | DEVICE_STATE_DEBUGGERATTACHED;
        }
        return deviceState;
    }
    ...
    public static int getBatteryVelocity(Context context, boolean powerConnected) {
        ...
        if (!powerConnected) {
            return DEVICE_STATE_ISSIMULATOR;
        }
        ...
        return DEVICE_STATE_JAILBROKEN;
    }
    ...
    public static String hexify(byte[] bytes) {
        char[] hexChars = new char[(bytes.length * DEVICE_STATE_JAILBROKEN)];
        for (int i = 0; i < bytes.length; i += DEVICE_STATE_ISSIMULATOR) {
            int v = bytes[i] & Type.ANY;
            hexChars[i * DEVICE_STATE_JAILBROKEN] = HEX_VALUES[v >>> DEVICE_STATE_DEBUGGERATTACHED];
            hexChars[(i * DEVICE_STATE_JAILBROKEN) + DEVICE_STATE_ISSIMULATOR] = HEX_VALUES[v & 15];
        }
    ...
    public static byte[] dehexify(String string) {
        ...
        byte[] data = new byte[(len / DEVICE_STATE_JAILBROKEN)];
        for (int i = 0; i < len; i += DEVICE_STATE_JAILBROKEN) {
            data[i / DEVICE_STATE_JAILBROKEN] = (byte) ((Character.digit(string.charAt(i), DEVICE_STATE_VENDORINTERNAL) << DEVICE_STATE_DEBUGGERATTACHED) + Character.digit(string.charAt(i + DEVICE_STATE_ISSIMULATOR), DEVICE_STATE_VENDORINTERNAL));
        }
        return data;
    }
    ...
    public static boolean isNullOrEmpty(String s) {
        return (s == null || s.length() == 0) ? true : CLS_TRACE_DEFAULT;
    }
    ...
    public static String padWithZerosToMaxIntWidth(int value) {
        ...
        Object[] objArr = new Object[DEVICE_STATE_ISSIMULATOR];
        objArr[0] = Integer.valueOf(value);
        return String.format(Locale.US, "%1$10s", objArr).replace(' ', '0');
    }
    ...
    public static String logPriorityToString(int priority) {
        switch (priority) {
            case DEVICE_STATE_JAILBROKEN /*2*/:
                return LOG_PRIORITY_NAME_VERBOSE;
            case Protocol.GGP /*3*/:
                return LOG_PRIORITY_NAME_DEBUG;
            case DEVICE_STATE_DEBUGGERATTACHED /*4*/:
    ...
    public static void finishAffinity(Activity activity, int resultCode) {
        if (activity != null) {
            if (VERSION.SDK_INT >= DEVICE_STATE_VENDORINTERNAL) {
